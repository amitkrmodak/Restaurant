 ...
 * Amit Kumar Modak 
 */
public class MenuCatagoryDaoImpl implements MenuCatagoryDao{
...
//	}
	...
	public MenuCatagoryDaoImpl() {
		getMenuCatagory();
	}

	public MenuCatagoryDaoImpl(int choice) {
		switch(choice){
		 case 1:
			 addMenuCatagory();
			 break;
	...
	@Override
	public MenuCatagory getDataById(int id)		// use in Menu Item
	{
		Session session = HibernetUtil.getSessionFactory().openSession();
	...
	@Override
	public void deleteMenuCatagory() {
		getMenuCatagory();
		showMenuCatagory();
		if (menu_catagory_list.size() != 0) {
	...
	@Override
	public int isCatagoryExists(String new_catagory) {
		Session session = HibernetUtil.getSessionFactory().openSession();
